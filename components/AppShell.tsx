    ...
    <div className="flex h-screen overflow-hidden bg-[#0a1628]">
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        <header className="flex-shrink-0 flex items-center gap-3 px-4 py-2.5 border-b border-slate-700/50 bg-[#0f1b2d]">
          <button onClick={() => setControlsOpen(o => !o)} className="p-1.5 rounded hover:bg-slate-700 text-slate-400 hover:text-white transition-colors">
          ...
          {draftedCount > 0 && (
            <div className="ml-2 flex items-center gap-2 bg-slate-800 rounded px-2 py-1 text-xs text-slate-300">
              ...
              <button onClick={() => setDraftedIds(new Set())} className="text-slate-500 hover:text-white ml-1">✕</button>
            </div>
          )}
        </header>
        ...
        <main className="flex-1 overflow-hidden">
          {activeTab === 'draft' && (
            <DraftBoard ranked={ranked} onSelect={setSelectedPlayer} onToggleDraft={toggleDrafted} draftedIds={draftedIds} />
          )}
          {activeTab === 'team' && (
            <TeamTracker myTeam={ranked.filter(p => p.drafted)} ranked={ranked} onSelect={setSelectedPlayer} draftPick={draftedCount + 1} />
          )}
          {activeTab === 'pool' && (
            <FullPool ranked={ranked} onSelect={setSelectedPlayer} onToggleDraft={toggleDrafted} draftedIds={draftedIds} />
          )}
      ...
      {selectedPlayer && (
        <PlayerDrawer player={selectedPlayer} settings={settings} onClose={() => setSelectedPlayer(null)} onToggleDraft={toggleDrafted} />
      )}
    </div>
  )
}
